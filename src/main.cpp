...
{
    // Credit: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    int64 nSubsidy = 5 * COIN;
 	  ...
 	  if(nHeight < 2599870 )           // next stable 5 coin
      {
              int64 kolikToho = 100000; // maximum reward of block
              kolikToho = ceil( kolikToho - nHeight/26 );
              nSubsidy = kolikToho * COIN;
...
static const int64 nTargetTimespan = 10 * 60; // Credit: 10 min
static const int64 nTargetSpacing = 2 * 60; // Credit: 2 minutes
static const int64 nInterval = nTargetTimespan / nTargetSpacing; // dif change every 5 blocks
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    // BIP16 didn't become active until 13.2.14
    int64 nBIP16SwitchTime = 1392246573;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Credit: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Credit: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CreditMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CreditMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CreditMiner(CWallet *pwallet)
{
    printf("CreditMiner started\n");
    ...
    try { loop {
        ...
        printf("Running CreditMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CreditMiner, pwallet));
}
